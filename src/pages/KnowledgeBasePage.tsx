...
const KnowledgeBasePage = () => {
      ...
      <div className="grid grid-cols-2 gap-4">
        {categories.map((cat) => (
          <button 
            ...
            className="bg-white p-6 rounded-[32px] border border-slate-200/50 shadow-sm hover:shadow-xl transition-all text-left group"
          >
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{cat.count} Artículos</p>
          </button>
        ))}
      </div>
      ...
      <section>
        <div className="flex justify-between items-center mb-6 px-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-black text-slate-900 uppercase tracking-widest">Tendencias Ahora</h2>
          </div>
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-3 py-1 rounded-full">Actualizado</span>
        </div>
        
        <div className="grid grid-cols-1 gap-4">
          {[
            { title: "¿Cómo descargar mi ticket PDF?", icon: <FileText className="w-5 h-5" />, tag: "TOP", color: "text-blue-600 bg-blue-50" },
            { title: "¿Puedo transferir mi entrada?", icon: <Users className="w-5 h-5" />, tag: "POPULAR", color: "text-purple-600 bg-purple-50" },
            { title: "Métodos de pago aceptados", icon: <CreditCard className="w-5 h-5" />, tag: "INFO", color: "text-emerald-600 bg-emerald-50" },
          ].map((article, idx) => (
            <button 
              key={idx}
              onClick={() => handleArticleClick(categories[1].articles[idx % categories[1].articles.length])}
              className="group flex items-center gap-4 p-5 bg-white rounded-[32px] border border-slate-200/50 shadow-sm hover:shadow-xl hover:border-primary/20 transition-all text-left animate-in slide-in-from-bottom-2 duration-300"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="text-[14px] font-bold text-slate-800 group-hover:text-primary transition-colors truncate">
                  {article.title}
                </h3>
              </div>
      ...
      <div className="space-y-3">
        {selectedCategory.articles.map((article: any, idx: number) => (
          <button 
            ...
            className="w-full flex items-center gap-4 p-5 bg-white rounded-3xl border border-slate-200/50 shadow-sm hover:shadow-md transition-all text-left group"
          >
            ...
            <span className="flex-1 text-[14px] font-bold text-slate-700 group-hover:text-slate-900 transition-colors">{article.title}</span>
  ...
  const renderArticle = () => (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white rounded-[32px] p-8 border border-slate-200/50 shadow-sm space-y-8">
        <div className="flex items-center gap-4 py-4 border-b border-slate-50">
          <div className="flex items-center gap-2 text-[10px] text-slate-400 font-black uppercase tracking-widest bg-slate-50 px-3 py-1.5 rounded-full">
        ...
        <div className="prose prose-slate prose-sm max-w-none">
          ...
          <p className="text-slate-600 leading-relaxed text-[15px] font-medium mt-4">
      ...
      <div className="bg-slate-900 pt-12 pb-20 px-6 relative overflow-hidden transition-all duration-500">
        ...
        <div className="max-w-2xl mx-auto relative z-10">
          <button 
            ...
            className="mb-8 p-2.5 rounded-2xl bg-white/10 border border-white/10 text-white backdrop-blur-md active:scale-90 transition-all"
          >
          ...
          {view === 'home' && (
            <div className="relative animate-in zoom-in-95 duration-500">
              ...
              <Input 
                ...
                className="pl-12 h-16 rounded-[24px] border-none bg-white text-slate-900 shadow-2xl shadow-black/20"
        ...
        {view !== 'article' && (
          <div className="bg-primary/5 border border-primary/10 rounded-[32px] p-8 text-center space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="font-bold text-slate-900">¿No encuentras lo que buscas?</h3>
              <p className="text-[12px] text-slate-500 font-medium">Nuestro equipo está disponible para ayudarte.</p>
            </div>
            <button 
